...
def puntuacion_max():
    puntuacion = datos_empresa()
    puntuacion_max = np.amax(puntuacion[:,3].astype(int))
    return puntuacion_max

# determinamos el indice de la puntuacion maxima
def indice_puntuacion_max():
    puntuacion = datos_empresa()
    indice_puntuacion_max = np.argmax(puntuacion[:,3].astype(int))
    return indice_puntuacion_max

# imprimimos por pantalla el resultado
def imprimir_resultados():
    puntuacion_maxima = puntuacion_max()
    indice_puntuacion_maxima = indice_puntuacion_maxima()
